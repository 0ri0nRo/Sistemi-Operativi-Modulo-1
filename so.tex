...
Definiamo come \textbf{stato sicuro} una situazione in cui si ha una sequenza sicura per i
thread. \par
Uno \textbf{stato insicuro} non implica la presenza di un deadlock, poiché potrebbe verificarsi che alcuni thread non richiedano simultaneamente il massimo numero di risorse necessarie dichiarato.\par
Tramite tale policy, una risorsa viene ceduta ad un thread solo se il nuovo stato è sicuro, altrimenti tale thread rimarrà in attesa che la risorsa sia disponibile in modo sicuro, impedendo quindi la creazione di possibili attese circolari.
\pagebreak
\subsubsection{Archi di pretesa}
Oltre alla formula numerica precedentemente vista, viene utilizzata una variante del RAG avente una tipologia aggiuntiva di arco, ossia gli \textbf{archi di pretesa}, indicanti che il thread $t{i}$ potrebbe richiedere la risorsa $r_{j}$ in futuro.\par
