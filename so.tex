...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.6\textwidth]{im/logo_sapienza_new.jpg}
    \label{fig:logo}
\end{figure}
...
Un \textbf{thread} è un'unità base dell'utilizzo della CPU e consiste in un contatore di programma, uno stack, e un insieme di registri oltre che un proptio ID. Ogni processo definisce le risorse "globali" (ossia lo spazio d'indirizzamento, le istru- zioni da eseguire, i dati, le risorse, \dots) mentre ogni suo thread definisce un singolo stream di esecuzione all'interno del processo stesso.
...
\begin{itemize}
    \item Reattività, poiché un processo potrebbe fornire una risposta in modo rapido mentre tutti gli altri thread sono bloccati o rallentati a causa dell'intenso uso della CPU
    \item Condivisione delle risorse "globali" come codice, dati e spazio di indirizzamento
    \item Economicità, poiché creare e gestire thread è più veloce rispetto alla creazione e gestione dei processi
    \item Scalabilità, poiché un processo single-thread può essere eseguito su un singolo core, mentre ogni thread di un processo multithread può essere suddiviso tra tutti i core disponibili della CPU (solo per architetture multi-core)
\end{itemize}
...
\begin{itemize}
    \item Parallelismo dei dati: suddivide i dati tra più core (thread) e
    esegue la stessa attività su ciascun blocco di dati
    \item Parallelismo dei compiti: suddivide i diversi compiti da eseguire tra i diversi core e li esegue contemporaneamente
    \item In pratica, nessun programma è mai suddiviso unicamente dall'uno o dall'altro di questi, ma piuttosto da una sorta di combinazione ibrida
\end{itemize}
...
\begin{itemize}
    ...
    \item Nessun cambio di contesto effettivo
\end{itemize}
\pagebreak
Tra gli \textbf{svantaggi}:
\begin{itemize}
    ...
    \item Richiede chiamate di sistema non bloccanti, altrimenti
    tutti i thread all'interno di un processo devono attendere
\end{itemize}
\begin{figure}[hbt]
    \begin{center}
...
\begin{figure}[hbt]
    \begin{center}
        \includegraphics[width=0.5\textwidth,keepaspectratio]{{im/cs1}}
        \caption{The Critical Section Problem}    
    \end{center}
\end{figure}
...
\begin{itemize}
    ...
    \item  Garantire la vivacità significa che qualcuno dovrebbe comprare del latte (cioè, l'opzione dove sia \textit{Bob} che \textit{Carla} non fanno nulla è sicuramente sicuro ma indesiderabile)
...
\begin{itemize}
    \item Acquisire sempre il blocco prima di accedere ai dati condivisi
    \item Rilasciare sempre il blocco dopo aver terminato con i dati condivisi • Il blocco deve essere inizialmente libero
    \item Solo un processo/thread può acquisire il lock, gli altri aspetteranno!
\end{itemize}

Poiché lo scheduler della CPU prende il controllo solo a seguito di eventi interni, ossia quando il thread in esecuzione lascia il controllo della CPU, oppure a seguito di eventi esterni, per evitare problemi all'interno dei settori critici è necessario impedire lo scheduling durante il rilascio o l'acquisizione di un lock. Per implementare le \textbf{primitive} ad alto livello relative alla sincronizzazione, quindi, è necessario del supporto harware al basso livello che possa rendere tali \textbf{operazioni atomiche}\footnote{Un'operazione viene detta atomica se essa non può essere interrotta in alcun modo, impedendo quindi che possa avvenire un context switch durante la sua esecuzione.}, in particolare tramite la \textbf{disabilitazione degli interrupt}\footnote{La prima soluzione a supporto hardware per poter implementare la sincronizzazione, corrisponde alla disabilitazione degli interrupt mentre un thread acquisisce o rilascia un lock} o l'uso di istruzioni atomiche.


\begin{figure}[hbt]
    \begin{center}
        \includegraphics[width=0.5\textwidth,keepaspectratio]{{im/lock}}
        \caption{The Critical Section Problem}    
    \end{center}
\end{figure}

Sui sistemi a CPU singola, possiamo impedire allo scheduler di prendere il sopravvento: 
\begin{itemize}
    \item eventi interni imponendo ai thread di non richiedere alcuna operazione di I/O durante un
    sezione critica
    \item evento esterno per disabilitare gli interrupt (ovvero, dire all'HW di ritardare la gestione di qualsiasi evento esterno fino a quando il thread corrente non è terminato con la sezione critica)
\end{itemize}
...
In sintesi supponiamo di avere una sola variabile condivisa (lock), inizialmente a 0. Quando un processo vuole entrare nella sua regione critica, prima controlla il lock. Se è O, il processo lo imposta a 1 ed entra nella regione critica. Se è già a 1, il processo aspetta finché non vale O. In questo modo O significa che nessun processo è nella sua regione critica, 1 che un qualche processo è nella sua regione critica.\par
Sfortunatamente questa idea presenta esattamente lo stesso \textbf{difetto} constatato nella directory di spool di stampa. Supponiamo che un processo legga il lock e veda che è 0. Prima che riesca a metterlo a 1, viene eseguito un altro processo schedulato e imposta il lock a 1.\par
