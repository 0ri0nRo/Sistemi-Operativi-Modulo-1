...
Intuitivamente, una condizione in cui due o più thread sono in attesa di un evento che può essere generato solo dagli stessi thread. Si può verificare quando più thread competono per un numero finito di risorse:
\begin{itemize} 
...
\begin{itemize}
    ...
    \item I bordi possono essere di 2 tipi:
\end{itemize}
    \begin{itemize}
        \item Request Edge à un edge diretto $(t_{i}, r_{j})$ indica che $t_{i}$ ha richiesto $r_{j}$, ma non ancora acquisita
        \item Assignment Edge à un edge diretto $(r_{j}, t_{i})$ indica che il sistema operativo ha assegnato $r_{j}$ a $t_{i}$
\end{itemize}
...
\begin{itemize}
    ...
    \begin{figure}[hbt]
        \begin{center}
            \includegraphics[width=0.5\textwidth,keepaspectratio]{{im/rag}}
            \caption{}    
        \end{center}
    \end{figure}
    ...
    \begin{figure}[hbt]
        \begin{center}
            \includegraphics[width=0.5\textwidth,keepaspectratio]{{im/rag2}}
            \caption{}    
        \end{center}
    \end{figure}
    \pagebreak
...
\end{itemize}
Un deadlock si verifica quando una delle quattro condizioni:
\begin{itemize}
    \item \textbf{Mutua esclusione}: rendere le risorse condivisibili (sebbene non tutte possano essere condivise)
    \item \textbf{Hold and Wait}  un thread non può trattenere una risorsa quando ne richiede un'altra (imporre richieste da effettuare tutte in una volta)
    \item \textbf{No Preemption}  se un thread richiede una risorsa che non può essere allocata ad esso, il sistema operativo preemption (rilascia) tutte le risorse che il thread è già in possesso. Problema: non tutte le risorse possono essere facilmente anticipate (ad es. stampanti)
    \item \textbf{Attesa circolare}: imporre un ordinamento (ovvero una numerazione) alle risorse e applicarlo
    richiederli in tale ordine
\end{itemize}
