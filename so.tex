...
I processi possono richiedere la propria terminazione effettuando la chiamata di sistema \texttt{exit}, tipicamente restituendo un \textit{int}. Questo \textit{int} viene passato al genitore se sta eseguendo un'attesa. Solitamente è 0 in caso di completamento con successo e un po' diverso da zero in caso di problemi.\\
...
\begin{itemize} 
    ...
    \item In risposta a un comando kill o ad un altro interrupt di processo non gestito
\end{itemize}
...
Quando un processo termina, tutte le sue risorse di sistema vengono liberate, i file aperti svuotati e chiusi, ecc. Lo stato di terminazione del processo dei tempi di esecuzione vengono restituiti al genitore se questo è in attesa che il figlio termini o eventualmente \texttt{init} se il processo diventa orfano. \\
I processi che stanno tentando di terminare ma non possono perché il loro genitore non li sta aspettando sono chiamati zombie eventualmente ereditati da \texttt{init} come orfani e uccisi.
...
\begin{itemize}
    \item Uno scheduler a lungo termine viene eseguito raramente ed è tipico di un sistema batch o di un sistema molto carico
    \item Uno scheduler a breve termine viene eseguito molto frequentemente (circa ogni 100 millisecondi) e deve sostituire molto rapidamente un processo dalla CPU e inserirne un altro
    \item Alcuni sistemi utilizzano anche uno scheduler a medio termine: quando i carichi del sistema diventano elevati, questo scheduler consente ai lavori più piccoli e più veloci di terminare rapidamente e liberare il sistema
\end{itemize}
...
\begin{itemize}
    \item Processi indipendenti operano contemporaneamente su un sistema e possono né influenzare né essere influenzati da altri processi
    \item I processi cooperanti possono influenzare o essere influenzati da altri processi al fine di raggiungere un compito comune
\end{itemize}
...
Esistono però tre problemi chiave da risolvere e nello scambio di messaggi è necessario:
\begin{itemize}
